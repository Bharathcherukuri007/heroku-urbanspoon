...
try{
    ...
    ResultSet rs=st.executeQuery("select * from iorder where city='"+k+"'");
    while(rs.next()){
    	 String hotelname=rs.getString(2),itemname=rs.getString(3);
...
<div class="row">
  ...
  <div class="card">
      ...
      <h1 name="hotelname"><%=rs.getString(2) %></h1>
      <p name="item"><%=rs.getString(3) %> </p>
      <p  name="price"><%=rs.getInt(1) %></p>
      
     
     
      <p><button onclick="location.href='cart.jsp?price=<%=rs.getInt(1) %>&hotel_name=<%=rs.getString(3)%>&item_name=<%=rs.getString(2)%> '">View More</button></p>
    </div>
  </div>
 <%
